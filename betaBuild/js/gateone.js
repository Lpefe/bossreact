...
//                     console.log('Running: ' + moduleObj.__name__ + '.postInit()');
    ...
    GateOne.Base.update(GateOne.Logging, {
        init: function() {
             ...
             */
            // The default is to send all client-side log LocaleMsg to the server but this can be disabled by setting `GateOne.prefs.logToServer = false`
        ...
        log: function(msg, level, destination) {
             ...
             * If ``null`` (as opposed to undefined), level info will not be included in the log message.
             If *destination* is given (must be a function) it will be used to log LocaleMsg like so: ``destination(message, levelStr)``.  The usual conversion of *msg* to *message* will apply.
        ...
        logToServer: function(msg, /*opt*/level) {
            /**:GateOne.Logging.logToServer(msg[, level])
             Sends the given log *msg* to the Gate One server.  Such LocaleMsg will end up in 'logs/gateone-client.log'.
             */
            var message = {
                "message": msg,
                "level": level || "info"
        ...
        addDestination: function(name, dest) {
            /**:GateOne.Logging.addDestination(name, dest)
             ...
             .. tip:: With the right function you can send client log LocaleMsg *anywhere*.
             */
    ...
    GateOne.Logging.destinations = { // Default to console logging.
        ...
        'server': GateOne.Logging.logToServer // Sends log LocaleMsg to the server to be saved/recored in logs/gateone-client.log
...
// NOTE:  The actual default actions are assigned at the end of the module.  I put this all the way up here because it flows better in the documentation.
    ...
    GateOne.Net.binaryBuffer = {}; // Incoming binary data LocaleMsg get stored here like so:
    ...
    GateOne.Base.update(GateOne.Net, {
        ...
        log: function(message) {
            /**:GateOne.Net.log(message)
             ...
             When developing a new action, you can test out or debug your server-side LocaleMsg by attaching the respective action to :js:func:`GateOne.Net.log` like so:
             .. code-block:: javascript
             GateOne.Net.addAction('my_action', GateOne.Net.log);
             Then you can view the exact LocaleMsg received by the client in the JavaScript console in your browser.
             .. tip:: Executing ``GateOne.Logging.setLevel('DEBUG')`` in your JS console will also log all incoming LocaleMsg from the server (though it can be a bit noisy).
             */
        ...
        onMessage: function(evt) {
            /**:GateOne.Net.onMessage(evt)
             ...
             This gets attached to :js:attr:`GateOne.ws.onmessage` inside of :js:func:`~GateOne.Net.connect`.  It takes care of decoding (`JSON <https://developer.mozilla.org/en/JSON>`_) LocaleMsg sent from the server and calling any matching :js:attr:`~GateOne.Net.actions`.  If no matching action can be found inside ``event.data`` it will fall back to passing the message directly to :js:func:`GateOne.Visual.displayMessage`.
             */
            ...
            if (evt.data[0] == '{') {
                ...
                // Non-JSON LocaleMsg coming over the WebSocket are assumed to be errors, display them as-is (could be handy shortcut to display a message instead of using the 'notice' action).
...
//         logDebug('applyTransform(' + typeof(obj) + ', ' + transform + ')');
        ...
        displayMessage: function(message, /*opt*/timeout, /*opt*/removeTimeout, /*opt*/id, /*opt*/noLog) {
            ...
            var u = go.Utils,
                ...
                logTemp = u.createElement('div'), // Used to strip HTML from LocaleMsg before we log them (because they're hard to read otherwise)
            ...
            if (message == go.Visual.lastMessage) {
                // Only display LocaleMsg every two seconds if they repeat so we don't spam the user.
                if (timeDiff < 2000) {
                    return;
                }
            }
            if (!noLog) {
                ...
                logInfo('Message: ' + logTemp.textContent); // Useful for looking at previous LocaleMsg
            }
